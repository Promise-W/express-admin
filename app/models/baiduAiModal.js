...
class modal {
  ...
  getToken() {
    return new Promise( async (resolve, reject) => {
      await exec(`curl 'https://aip.baidubce.com/oauth/2.0/token?grant_type=client_credentials&client_id=${apiKey}&client_secret=${apiSecret}'`, async (error, stdout, stderr) => {
        if (error) {
